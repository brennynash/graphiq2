  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        ...
        // Filter blogs with topic "Knowledge"
        const knowledgeBlogs = data.filter(blog => blog.topic.toLowerCase() === 'knowledge');
        setBlogs(knowledgeBlogs);
      } catch (error) {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {blogs.length === 0 && (
        <div className="text-center text-gray-600">
          No knowledge posts available yet.
        </div>
      )}
    </div>
  );
