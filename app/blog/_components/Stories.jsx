  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        ...
        // Filter blogs with topic "Project Story"
        const storyBlogs = data.filter(blog =>
          blog.topic.toLowerCase() === 'project story' ||
          blog.topic.toLowerCase() === 'project-story' ||
          blog.topic.toLowerCase() === 'project stories'
        );
        setBlogs(storyBlogs);
      } catch (error) {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {blogs.length === 0 && (
        <div className="text-center text-gray-600">
          No project stories available yet.
        </div>
      )}
    </div>
  );
